...
class ScheduleListController: UIViewController {
  ...
  private var events = [Event]()
    ...
    //public let dataPersistence = DataPersistence
  ...
  //Querys the data persistence for the items
  ...
  private func update(oldEvent: Event, with newEvent: Event) {
    ...
    // call load items to update events array, it retrieves documents from the documents directory and append to our events array and reload the tableView
    loadItems()
  }
  ...
  @IBAction func editButtonPressed(_ sender: UIBarButtonItem) {
    ...
    //if this is optional you need to pass in some value but you dont want to pass in nil so you are going to default nil
    private func showCreateEventVC(_ event: Event? = nil) {
//  we need to use the storyboard to get an instance of the CreateEventController all view controllers has a property ca;led storyboard so if you have multiple story board it will help with having the identifier. So we will use an instance of storyboard
        guard let createEventController = storyboard?.instantiateViewController(identifier: "CreateEventController") as? CreateEventController else {
            fatalError("Could not downcast to CreateEventController")// this is considered a developer problem
        }
        //let createEC = CreateEventController() - creates an empty viewController without any outlets etc will crash if you are expecting your outlets to work if they exist in the storyboard
        ...
        //for updating an event we will inject(dependency injection) the selected event. true dependency injection you will need an initializer
        present(createEventController, animated: true)//we want o present it cause we want to slide it up
    }
}
